...
npm run build     # 本番ビルド
npm run test:e2e  # E2E (要: supabase start + dev server。fakeカメラで実行)
...
手順(Supabase本番設定 → Vercel → Stripe → TURN → 管理者作成)と
運用マニュアル(通報対応・バックアップ・障害対応)は `docs/09_operations.md` を参照。

> **注意**: 本リポジトリの開発環境ではDocker・外部APIキー・デプロイ先が
> 利用できなかったため、**公開URLの発行とE2E・実機のWebRTC相互接続は未検証**です。
> DBスキーマ・RLS・RPCロジックは素のPostgreSQL 16上で実地検証済み
