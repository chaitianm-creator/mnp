...
/// 今日の依頼(/daily-request)。お客様からの実依頼リスト + ぼうけんフィールド。
/// 練習クエストを3つクリアするまではロック(はじめてでも迷子にならない導線)。
/// (旧ホームのチェックリスト+ゲームフィールドをこの画面に引っ越し)
class DailyRequestPage extends ConsumerWidget {
  const DailyRequestPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final progress = ref.watch(userProgressProvider);

    if (!progress.dailyRequestUnlocked) {
      return Scaffold(
        appBar: AppBar(title: const Text('今日の依頼')),
        body: SafeArea(child: _LockedView(cleared: progress.practiceClearedCount)),
      );
    }
...
/// ロック画面: 解放条件と進み具合をやさしく伝える。
class _LockedView extends StatelessWidget {
  const _LockedView({required this.cleared});
  final int cleared;
    ...
    return ListView(padding: const EdgeInsets.all(20), children: [
      ...
      Center(
        child: Container(
          ...
          height: 96,
          decoration: BoxDecoration(
            color: const Color(0xFFD8CBB4),
            shape: BoxShape.circle,
            border: Border.all(color: KdColors.wood900, width: 3),
            boxShadow: const [
              BoxShadow(color: Color(0xFFB3A68E), offset: Offset(0, 4)),
            ],
          ),
          child: const Icon(Icons.lock, size: 46, color: Colors.white),
        ),
      ),
      const SizedBox(height: 18),
      Center(child: KdRibbonBanner('まだ ひみつの扉', fontSize: 14)),
      ...
      KdParchmentCard(
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            ...
            const Spacer(),
            Text('$cleared / 3',
                style: KdTheme.dot(size: 16, color: KdColors.pink700)
                    .copyWith(fontWeight: FontWeight.w700)),
          ]),
          const SizedBox(height: 8),
          KdProgressBar(value: cleared / 3),
          const SizedBox(height: 12),
          Text('お客様からの「本物の依頼」は、基礎を身につけてからのお楽しみ♪',
              style: Theme.of(context).textTheme.bodyMedium),
        ]),
      ),
      const SizedBox(height: 16),
